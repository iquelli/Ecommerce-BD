...
def register_product_post():
    try:
        ...
        return render_template(
            "error.html", error=e, context="product"
        )  # Renders a page with the error.
    finally:
        dbConn.commit()
        cursor.close()
        dbConn.close()
...
@app.route("/alterproduct", methods=["GET"])
def alter_product():
    try:
        dbConn = psycopg2.connect(DB_CONNECTION_STRING)
        cursor = dbConn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        ...
        if description:
            cursor.execute(
                "UPDATE products SET descriptiom = %s WHERE id = %s",
...
def register_supplier_post():
    try:
        ...
        return render_template(
            "error.html", error=e, context="supplier"
        )  # Renders a page with the error.
    finally:
        dbConn.commit()
        cursor.close()
...
def register_customer_post():
    try:
        ...
        return render_template(
            "error.html", error=e, context="customer"
        )  # Renders a page with the error.
    finally:
        dbConn.commit()
        cursor.close()
